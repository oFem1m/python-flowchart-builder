...
class Text extends Mesh {
  ...
  getCaretAtPoint(textRenderInfo: object, x: number, y: number): { x: number; y: number; height: number; charIndex: number };
  getSelectionRects(textRenderInfo: object, start: number, end: number): { left: number; top: number; right: number; bottom: number }[];
}
